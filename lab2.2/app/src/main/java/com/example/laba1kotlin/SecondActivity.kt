...
class SecondActivity : AppCompatActivity(), MyListAdapter.MyClickListener {
    ...
    },
    {
        ...
        val klaxon = Klaxon()
        val civilizations = arrayListOf<MyListAdapter.Civilization>()
            ...
            JsonReader(StringReader(jsonString)).use { reader ->
                reader.beginArray {
                    while (reader.hasNext()) {
                        if (flag)
                        {
                            val tech = klaxon.parse<MyListAdapter.Civilization>(reader)
                            civilizations.add(tech!!)
                        }
                        else
                        {
        ...
        recycleView.adapter = MyListAdapter(civilizations, this, this)
    }

    override fun onItemClick(item: MyListAdapter.Civilization, position: Int) {
